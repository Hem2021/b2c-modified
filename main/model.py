...
from common.nets.tsm.tsm_resnet import ResNetBackbone

# print("Location to Resnet : ", ResNetBackbone)  # This will print the location of the class definition
from common.nets.module import Pose2Feat, Aggregator, Classifier, Aggregator_f
...
def get_line_intersection(point0, point1, point2, point3):
    ...
    mask = (
        mask
        * (1 - s_numer_Positive * denomPositive)
        * (1 - s_numer_Negative * denomNegative)
    )
    ...
    mask = (
        mask
        * (1 - t_numer_Positive * denomPositive)
        * (1 - t_numer_Negative * denomNegative)
    )
...
def get_triangle_dense_area(xx, yy, point0, point1):
    ...
    area_ = torch.sqrt(
        mean_p * (mean_p - edge_a) * (mean_p - edge_b) * (mean_p - edge_c)
    )
    return area_
...
def get_quad_region(xx, yy, point0, point1, point2, point3):
    ...
    dense_area_012 = (
        get_triangle_dense_area(xx, yy, point0, point1)
        + get_triangle_dense_area(xx, yy, point1, point2)
        + get_triangle_dense_area(xx, yy, point2, point0)
    )
    dense_area_023 = (
        get_triangle_dense_area(xx, yy, point2, point3)
        + get_triangle_dense_area(xx, yy, point3, point0)
        + get_triangle_dense_area(xx, yy, point2, point0)
    )
    ...
    mask = (mask_area_012[:, :, :, :, None, None].float()) * mask_012.float() + (
        mask_area_023[:, :, :, :, None, None].float()
    ) * mask_023.float()
    mask[mask >= 1.0] = 1.0
    return mask
...
class Model(nn.Module):
    def __init__(
        ...
        self.skeleton_part = torch.LongTensor(skeleton).to(device).view(-1, 2)
    ...
    def render_gaussian_heatmap(self, pose_coord, pose_score):
        ...
        xx = xx[None, None, None, :, :].cuda().float()
        yy = yy[None, None, None, :, :].cuda().float()

        x = pose_coord[:, :, :, 0, None, None]
        y = pose_coord[:, :, :, 1, None, None]
        heatmap = (
            torch.exp(
                -(((xx - x) / cfg.hm_sigma) ** 2) / 2
                - (((yy - y) / cfg.hm_sigma) ** 2) / 2
            )
            * (pose_score[:, :, :, None, None] > cfg.pose_score_thr).float()
        )  # score thresholding
    ...
    def render_paf(self, pose_coord, pose_score):
        ...
        coord0 = pose_coord[
            :, :, self.skeleton_part[:, 0], :
        ]  # batch_size*frame_num, person_num, part_num, 2
        ...
        # make paf
        dist = vector[:, :, :, 0, None, None] * (
            xx - coord0[:, :, :, 0, None, None]
        ) + vector[:, :, :, 1, None, None] * (yy - coord0[:, :, :, 1, None, None])
        dist_t = torch.abs(
            vector_t[:, :, :, 0, None, None] * (xx - coord0[:, :, :, 0, None, None])
            + vector_t[:, :, :, 1, None, None] * (yy - coord0[:, :, :, 1, None, None])
        )
        mask1 = (dist >= 0).float()
        mask2 = (dist <= normalizer[:, :, :, 0, None, None]).float()
        mask3 = (dist_t <= cfg.paf_sigma).float()
        score0 = pose_score[:, :, self.skeleton_part[:, 0], None, None]
        score1 = pose_score[:, :, self.skeleton_part[:, 1], None, None]
        mask4 = (score0 >= cfg.pose_score_thr) * (
            score1 >= cfg.pose_score_thr
        )  # socre thresholding
        mask = (
            mask1 * mask2 * mask3 * mask4
        )  # batch_size*frame_num, person_num, part_num, cfg.input_hm_shape[0], cfg.input_hm_shape[1]
        paf = torch.stack(
            (
                mask * vector[:, :, :, 0, None, None],
                mask * vector[:, :, :, 1, None, None],
            ),
            3,
        )  # batch_size*frame_num, person_num, part_num, 2, cfg.input_hm_shape[0], cfg.input_hm_shape[1]

        # sum and normalize
        mask = torch.sum(mask, (1))
        mask[mask == 0] = -1
        paf = (
            torch.sum(paf, (1)) / mask[:, :, None, :, :]
        )  # batch_size*frame_num, part_num, 2, cfg.input_hm_shape[0], cfg.input_hm_shape[1]
        paf = paf.view(
            paf.shape[0], paf.shape[1] * paf.shape[2], paf.shape[3], paf.shape[4]
        )
        return paf
    ...
    def render_limb_gaussian_heatmap(self, pose_coord, pose_score):
        ...
        xx = xx[None, None, None, :, :].cuda().float()
        yy = yy[None, None, None, :, :].cuda().float()  # 5: 1, 1, 1, H,W
        ...
        coord0 = pose_coord[
            :, :, self.skeleton_part[:, 0], :
        ]  # 4: batch_size*frame_num, person_num, part_num, 2-(x,y)
        coord1 = pose_coord[
            :, :, self.skeleton_part[:, 1], :
        ]  #  4: batch_size*frame_num, person_num, part_num, 2-(x,y)
        vector = (
            coord1 - coord0
        )  #  4: batch_size*frame_num, person_num, part_num, 2-(x,y)

        dist_start = (xx - coord0[:, :, :, 0, None, None]) ** 2 + (
            yy - coord0[:, :, :, 1, None, None]
        ) ** 2  #  5: batch_size*frame_num, person_num, part_num,H,W
        dist_end = (xx - coord1[:, :, :, 0, None, None]) ** 2 + (
            yy - coord1[:, :, :, 1, None, None]
        ) ** 2  #  5: batch_size*frame_num, person_num, part_num,H,W
        dist_ab = (
            coord0[:, :, :, 0, None, None] - coord1[:, :, :, 0, None, None]
        ) ** 2 + (coord0[:, :, :, 1, None, None] - coord1[:, :, :, 1, None, None]) ** 2
        dist_ab[dist_ab < 1] = 1  #  5: batch_size*frame_num, person_num, part_num,1,1

        coeff = (
            (dist_start - dist_end + dist_ab) / 2.0 / dist_ab
        )  #  5: batch_size*frame_num, person_num, part_num,H,W

        a_dominate = coeff <= 0
        b_dominate = coeff >= 1
        seg_dominate = (
            1 - a_dominate.float() - b_dominate.float()
        )  # 5: batch_size*frame_num, person_num, part_num,H,W

        projection_x = (
            coeff * vector[:, :, :, 0, None, None] + coord0[:, :, :, 0, None, None]
        )
        projection_y = (
            coeff * vector[:, :, :, 1, None, None] + coord0[:, :, :, 1, None, None]
        )
        d2_line = (xx - projection_x) ** 2 + (
            yy - projection_y
        ) ** 2  # 5: batch_size*frame_num, person_num, part_num,H,W
        d2_seg = (
            a_dominate * dist_start + b_dominate * dist_end + seg_dominate * d2_line
        )

        heatmap = torch.exp(-d2_seg / 2.0 / cfg.hm_sigma**2) * (
            pose_score[:, :, self.skeleton_part[:, 0], None, None] > cfg.pose_score_thr
        )
    ...
    def render_limb_motion(self, batch_size, pose_coord, pose_score):
        ...
        y = torch.arange(cfg.input_hm_shape[0])  # [0,1,2, ..., 55]
        yy, xx = torch.meshgrid(y, x)  # 2: H,W
        xx = xx[None, None, None, None, :, :].cuda().float()
        yy = yy[None, None, None, None, :, :].cuda().float()  # 6: 1, 1, 1, 1, H,W
        # pose_coord = torch.LongTensor(pose_coord)
        # reshape pose_coord like 5: batch_size, frame_num, person_num, part_num, 2
        frame_num = int(pose_coord.shape[0] / batch_size)
        motion_sequence = (
            torch.LongTensor([[i, i + 1] for i in range(frame_num - 1)])
            .cuda()
            .view(-1, 2)
        )
        pose_coord = pose_coord.view(
            batch_size,
            frame_num,
            pose_coord.shape[1],
            pose_coord.shape[2],
        ...
        temp0 = pose_coord[
            :, motion_sequence[:, 0], :, :, :
        ]  # 5: batch_size, adj_num, person_num, part_num, 2
        temp1 = pose_coord[
            :, motion_sequence[:, 1], :, :, :
        ]  # 5: batch_size, adj_num, person_num, part_num, 2
        ...
        pose_score = pose_score.view(
            batch_size, frame_num, pose_score.shape[1], pose_score.shape[2]
        )
        ...
        line_cross_mask, line_cross_point = get_line_intersection(
            temp0_coord0, temp0_coord1, temp1_coord0, temp1_coord1
        )
        temp_cross_mask, temp_cross_point = get_line_intersection(
            temp0_coord0, temp1_coord0, temp0_coord1, temp1_coord1
        )
        ...
        none_quad_mask = get_quad_region(
            xx, yy, temp0_coord0, temp0_coord1, temp1_coord1, temp1_coord0
        )
        ...
        temp_quad_mask = get_quad_region(
            xx, yy, temp0_coord0, temp0_coord1, temp1_coord1_extend, temp1_coord0
        )
        temp_circle_mask = get_circle_region(
            xx, yy, temp1_coord0, temp1_coord1_extend, temp1_coord1
        )

        # case 3, velocity_line_coord0 = velocity_coord0, velocity_line_coord1 = velocity_coord1
        line_coord0_mask = get_circle_region(
            xx, yy, line_cross_point, temp0_coord0, temp1_coord0
        )
        line_coord1_mask = get_circle_region(
            xx, yy, line_cross_point, temp0_coord1, temp1_coord1
        )

        # fusion all case
        limb_motion_x = (
            none_cross_mask[:, :, :, :, None, None]
            * none_quad_mask
            * velocity_mean[:, :, :, :, 0, None, None]
            + temp_cross_mask[:, :, :, :, None, None]
            * (
                temp_quad_mask * velocity_coord0[:, :, :, :, 0, None, None]
                + temp_circle_mask * velocity_coord1[:, :, :, :, 0, None, None]
            )
            + co_cross_mask[:, :, :, :, None, None]
            * line_cross_mask[:, :, :, :, None, None]
            * (
                line_coord0_mask * velocity_coord0[:, :, :, :, 0, None, None]
                + line_coord1_mask * velocity_coord1[:, :, :, :, 0, None, None]
            )
        )
        limb_motion_y = (
            none_cross_mask[:, :, :, :, None, None]
            * none_quad_mask
            * velocity_mean[:, :, :, :, 1, None, None]
            + temp_cross_mask[:, :, :, :, None, None]
            * (
                temp_quad_mask * velocity_coord0[:, :, :, :, 1, None, None]
                + temp_circle_mask * velocity_coord1[:, :, :, :, 1, None, None]
            )
            + co_cross_mask[:, :, :, :, None, None]
            * line_cross_mask[:, :, :, :, None, None]
            * (
                line_coord0_mask * velocity_coord0[:, :, :, :, 1, None, None]
                + line_coord1_mask * velocity_coord1[:, :, :, :, 1, None, None]
            )
        )
        ...
        limb_motion = torch.cat(
            (limb_motion, limb_motion.mean(1)[:, None, :, :, :, :]), 1
        )
        # batch_size, frame_num, part_num, 2, H, W
        normlizer = torch.sqrt(
            limb_motion[:, :, :, 0, :, :] ** 2 + limb_motion[:, :, :, 1, :, :] ** 2
        )
        normlizer[normlizer == 0] = 1
        limb_motion = limb_motion / normlizer[:, :, :, None, :, :]
        # batch_size, frame_num, part_num, 2, H, W

        limb_motion = limb_motion.view(
            limb_motion.shape[0] * limb_motion.shape[1],
            limb_motion.shape[2] * limb_motion.shape[3],
            limb_motion.shape[4],
            limb_motion.shape[5],
        )
    ...
    def forward(self, inputs, targets, meta_info, mode):
        dark_input_video = inputs[
            "dark_video"
        ]  # batch_size, frame_num, 3, cfg.input_img_shape[0], cfg.input_img_shape[1]
        light_input_video = inputs[
            "light_video"
        ]  # batch_size, frame_num, 3, cfg.input_img_shape[0], cfg.input_img_shape[1]
        batch_size, video_frame_num = dark_input_video.shape[:2]
        dark_input_video = dark_input_video.view(
            batch_size * video_frame_num,
            3,
            cfg.input_img_shape[0],
            cfg.input_img_shape[1],
        )
        light_input_video = light_input_video.view(
            batch_size * video_frame_num,
            3,
            cfg.input_img_shape[0],
            cfg.input_img_shape[1],
        ...
        if cfg.mode == "rgb_only":
            video_feat = self.img_backbone(dark_input_video, skip_early=False)
            # print("from model.py > forward > video_feat: ", video_feat.shape)
...
def get_model(class_num, joint_num, skeleton, mode):
    ...
    pose_backbone = ResNetBackbone(
        cfg.pose_resnet_type, (cfg.frame_per_seg - 1) * cfg.pose_frame_factor + 1
    )
