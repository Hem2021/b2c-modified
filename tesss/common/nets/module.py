...
class Aggregator(nn.Module):
    def __init__(self):
        ...
        ## aggregation layer
        self.video_2_pose = make_convtrans3d_layers([self.img_resnet_dim, self.pose_resnet_dim], kernel=(1,1,1), stride=(1,1,1), padding=0)
        self.v2p_norm = nn.LayerNorm([self.pose_resnet_dim,  cfg.frame_per_seg , 7, 7])
        # self.pose_2_video = ([self.pose_resnet_dim,self.img_resnet_dim], kernel=1, stride=(1,1,2),  padding=0)
        # self.p2v_norm = nn.LayerNorm([self.img_resnet_dim,  cfg.frame_per_seg, 7, 7])
        self.pose_2_video = make_convtrans3d_layers([self.pose_resnet_dim,self.img_resnet_dim], kernel=(1,1,1), stride=(1,1,1),  padding=0)
        self.p2v_norm = nn.LayerNorm([self.img_resnet_dim,  cfg.frame_per_seg, 7, 7])
    ...
    def forward(self, video_feat, pose_feat):
        ...
        if cfg.pose_frame_factor > 1:
                       pose_feat = self.pose_temporal_conv(pose_feat)
        print("inside module.py > video_feat : ", video_feat.shape)
        v2p_feat = self.video_2_pose(video_feat)
        v2p_feat = self.v2p_norm(v2p_feat)
        print("inside module.py > v2p_feat : ", v2p_feat.shape)
        #B, C, T, H, W
        print("inside module.py > pose_feat : ", pose_feat.shape)
        p2v_feat = self.pose_2_video(pose_feat)
        p2v_feat = self.p2v_norm(p2v_feat)
        print("inside module.py > p2v_feat : ", p2v_feat.shape)
        ...
        pose_gate = None
        video_gate = None
        return feat, pose_gate, video_gate
...
class Aggregator_f(nn.Module):
    def __init__(self):
        ...
        self.v2p_gate =make_conv_layers([cfg.frame_per_seg, self.pose_frame_num], kernel=(1,1), stride=(1,1), padding=0,bnrelu_final=False)
        self.p2v_gate =make_conv_layers([self.pose_frame_num,cfg.frame_per_seg], kernel=1, stride=(1,1),  padding=0,bnrelu_final=False)
        ...
        # added by hemant. since there is no upsampling, so no need to use make_convtrans_layers ie. no trnaspose convolution
        self.video_2_pose =make_conv_layers([cfg.frame_per_seg, self.pose_frame_num], kernel=1, stride=(1,1), padding=0)
        self.v2p_norm = nn.LayerNorm([self.pose_frame_num, 7, 7])

        # self.pose_2_video = make_conv_layers([self.pose_frame_num,cfg.frame_per_seg], kernel=1, stride=(1,2),  padding=0)
        self.pose_2_video = make_conv_layers([self.pose_frame_num,cfg.frame_per_seg], kernel=1, stride=(1,1),  padding=0)
    ...
    def forward(self, video_feat, pose_feat):
        ...
        batch_size, video_feat_dim, video_feat_height, video_feat_width = video_feat.shape[0]//cfg.frame_per_seg, video_feat.shape[1],video_feat.shape[2], video_feat.shape[3]
        video_feat = video_feat.view(batch_size, cfg.frame_per_seg, video_feat_dim,  video_feat_height,  video_feat_width).permute(0,2,1,3,4)
        # print("INSIDE MODULE.PY> video_feat initially : ", video_feat.shape)
        #B, C, T, H, W
        batch_size, pose_feat_dim, pose_feat_height, pose_feat_width = pose_feat.shape[0]//self.pose_frame_num, pose_feat.shape[1], pose_feat.shape[2], pose_feat.shape[3]
        pose_feat = pose_feat.view(batch_size, self.pose_frame_num, pose_feat_dim, pose_feat_height, pose_feat_width).permute(0,2,1,3,4)
        # print("INSIDE MODULE.PY> pose_feat initially : ", pose_feat.shape)
        #B, C, T, H, W
        ...
        video_feat = video_feat*p2v_gated + p2v_feat*(1-p2v_gated)
        video_feat = video_feat.permute(0,2,1,3,4).reshape(-1,video_feat_dim, video_feat_height, video_feat_width)
        # print("INSIDE MODULE.PY> video_feat after aggregation : ", video_feat.shape)

        pose_feat = pose_feat*v2p_gated + v2p_feat*(1-v2p_gated)
        pose_feat = pose_feat.permute(0,2,1,3,4).reshape(-1,pose_feat_dim, pose_feat_height, pose_feat_width)
        # print("INSIDE MODULE.PY> pose_feat after aggregation : ", pose_feat.shape)

        #B*T, C, H, W
        video_feat = video_feat.mean((2,3))[:,:,None,None]
        # print("INSIDE MODULE.PY> video_feat at last : ", video_feat.shape)
        pose_feat  = pose_feat.mean((2,3))[:,:,None,None]
        # print("INSIDE MODULE.PY> pose_feat at last : ", pose_feat.shape)
        # aggregation
        feat = torch.cat((video_feat, pose_feat),1)
        print("INSIDE MODULE.PY> final returned feature dimension : ", feat.shape)
        # pose gate
