...
def load_video(path, frame_num, start_frame_idx):
    ...
    for i in range(cfg.frame_per_seg):
        ...
        print("path in load_video : ", path)
        # % is used to assign a name to img file
        img = load_img(osp.join(path, "%.6d.jpg" % (cur_frame_idx + 1)))  # 1-based
        # print("img : ", img)
        if len(cfg.video_shape) == 2:
            img = cv2.resize(img, (cfg.video_shape[1], cfg.video_shape[0]))  # resize to configure H X W
                                                                             # in cv2, W is used first followed by H
            ...
            if width > height:
                video_shape = (
                    cfg.video_shape[0],
                    int(width / height * cfg.video_shape[0]),
                )  # height, width
            else:
                video_shape = (
                    int(height / width * cfg.video_shape[0]),
                    cfg.video_shape[0],
                )  # height, width
            img = cv2.resize(img, (video_shape[1], video_shape[0]))
        video.append(img)
        video_frame_idxs.append(cur_frame_idx)
        # frame_per_seg X H X W X channel_num(3)
    video = np.array(video).reshape(
        cfg.frame_per_seg, video_shape[0], video_shape[1], 3
    )
    ...
    pose_frame_idxs = [
        ...
        for i in range(
            start_frame_idx,
            start_frame_idx + (cfg.frame_per_seg - 1) * cfg.frame_interval + 1,
            cfg.frame_interval // cfg.pose_frame_factor,
        )
    ]
...
def process_skeleton(
    ...
    if do_flip:
        ...
        for pair in flip_pairs:
            pose_coords[pair[0], :], pose_coords[pair[1], :] = (
                pose_coords[pair[1], :],
                pose_coords[pair[0], :].copy(),
            )
            pose_scores[pair[0]], pose_scores[pair[1]] = (
                pose_scores[pair[1]],
                pose_scores[pair[0]].copy(),
            )
    pose_coords_xy1 = np.concatenate(
        (pose_coords, np.ones_like(pose_coords[:, 0:1])), 1
    )
    pose_coords = np.dot(img2aug_trans, pose_coords_xy1.transpose(1, 0)).transpose(
        1, 0
    )[:, :2]

    # transform to input heatmap space
    pose_coords[:, 0] = (
        pose_coords[:, 0] / cfg.input_img_shape[1] * cfg.input_hm_shape[1]
    )
    pose_coords[:, 1] = (
        pose_coords[:, 1] / cfg.input_img_shape[0] * cfg.input_hm_shape[0]
    )
...
def get_aug_config(video_shape):
    ...
    scale_factor = [1, 0.875, 0.75, 0.66]
    scale = []
    ...
    width, height = int(scale[0] * cfg.input_img_shape[1]), int(
        scale[1] * cfg.input_img_shape[0]
    )
...
def augmentation(video, data_split):
    ...
    video_aug = np.zeros(
        (frame_num, cfg.input_img_shape[0], cfg.input_img_shape[1], 3), dtype=np.float32
    )
    for i in range(frame_num):
        video_aug_before_resize, trans, inv_trans = generate_patch_image(
            video[i], bbox, do_flip, cfg.input_img_shape
        )
        video_aug[i] = cv2.resize(
            video_aug_before_resize, (cfg.input_img_shape[1], cfg.input_img_shape[0])
        )
    video = video_aug
...
def generate_patch_image(cvimg, bbox, do_flip, out_shape):
    ...
    trans = gen_trans_from_patch_cv(
        bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0]
    )
    ...
    inv_trans = gen_trans_from_patch_cv(
        bb_c_x, bb_c_y, bb_width, bb_height, out_shape[1], out_shape[0], inv=True
    )
...
def gen_trans_from_patch_cv(
    c_x, c_y, src_width, src_height, dst_width, dst_height, inv=False
):
    src_w = src_width
    src_h = src_height
