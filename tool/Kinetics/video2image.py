...
for data_split in ('train_256', 'val_256'):
    seq_path_list = glob(osp.join(video_root_path, data_split, '*'))
    ...
    for seq_path in tqdm(seq_path_list):
        ...
        for video_path in video_path_list:
            ...
            # make folders
            os.makedirs(osp.join(frame_save_path, data_split, seq_name), exist_ok=True)
            os.makedirs(osp.join(frame_save_path, data_split, seq_name, video_name), exist_ok=True)
            
            # save frames from the video
            img_path = osp.join(frame_save_path, data_split, seq_name, video_name, '%06d.jpg')
